...
describe('awsAcccountValidator', function () {
  ...
  let data;
  let awsCredentials = {
    getCredentialsForRole: sinon.stub().returns(Promise.resolve({}))
  };
  ...
  beforeEach(() => {
    ...
    sut = proxyquire(
      'commands/validators/awsAccountValidator',
      {
        'modules/amazon-client/awsCredentials': awsCredentials
      }
    );
  });
  ...
  describe('Role ARNs without the required privileges', () => {
    beforeEach(() => {
      ...
      awsCredentials.getCredentialsForRole = sinon.stub().throws(new Error());
    });
